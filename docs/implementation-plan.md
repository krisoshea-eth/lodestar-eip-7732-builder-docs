...
**Status:** Reviewed through 13 September 2026. Nico's confirmed direct-Engine direction and the [direct-Engine working plan](direct-engine-working-plan.md) control conflicts with the historical BN-mediated sections below. API-02, TEST-01, the ledger, preference tracker, Marko's store and policy, and their contribution PRs have merged. Naming follow-up #10063 has also merged. #9958, #10054, #9979, #9980, #9982, shared events #10064 and API cancellation #10065 are out of draft for maintainer review, not approved for merge. #10054 still has Changes Requested on GitHub and awaits Nazar's re-review after the correction. #9979/#9980/#9982 passed critical re-review and exact-head checks; their descriptions identify the component files separately from the already-merged ledger shown in the historical diff. #9973, #9978 and #9981 remain drafts dependent on open #9958. Fork #77 is a resolved-input integration draft; fork #80 remains tied to the open payload-attributes proposal. LOD-76/77/78 own the remaining input and runtime work, without an agreed assignee. The [13 September reconciliation](reviews/2026-09-13-reconciliation.md) records the latest heads, review replies awaiting approval, conflict resolution and evidence limits.

The later 12 September [input experiment](builder-input-contract.md) starts LOD-76 on `krisoshea/input-consumer`, separately from fork #77's existing PR. It consumes actual typed events through a bounded head-only Gloas path, using the existing `head_v2` roots and #80's provisional finality-hash transport. This is fork-only implementation evidence, not completed LOD-76/77/78 or an accepted input specification. Runtime event subscription, Builder/CLI construction and reveal wiring still need implementation.
...
The board preserves completed foundation work. API-02, TEST-01, BID-LEDGER-01, PREF-01, STORE-WIRING-01 and BID-POLICY-BASE-01 are now **Done** for their merged scopes. PAYLOAD-SOURCE-01, BID-PUBLISH-01, SELECT-MATCH-01 and REV-PUBLISH-01 are **In Review**; the source-dependent service drafts and resolved-input integration remain in progress. LOD-89 is assigned to Kris and **Done** through the merged naming-only [#10063](https://github.com/ChainSafe/lodestar/pull/10063). ENV-02 remains **In Review** pending independent reproduction, with outreach paused. LOD-87 is assigned to Kris and In Review through shared-subscription PR #10064. Its independent API cancellation companion #10065 is ready and tracked in REL-01. LOD-76 is In Progress for the fork-only input experiment; its final contract and broader ownership remain open. LOD-77/78 and preference recovery LOD-88 remain backlog and unassigned. Landed prerequisites and pure helpers narrow a consuming issue but do not automatically complete its integrated outcome. Linear and its GitHub mirrors carry the current per-issue status.
