...
The [input experiment](builder-input-contract.md) progresses LOD-76/77 on `krisoshea/input-consumer`, separately from fork #77's existing PR. It consumes typed events through a bounded head-only Gloas path, using existing `head_v2` roots and #80's provisional finality-hash transport. Explicit programmatic `bidRuntime` options now let `Builder.init()` construct the bid services and connect the shared stream. CLI/Engine transport construction, recovery, Heze inputs and selection/reveal wiring remain unfinished. Its 188 targeted tests do not establish a complete real BN/EL lifecycle or an accepted input specification.
