...
| `D-03` | Runtime boundary | **Updated 2 September.** The working baseline is a standalone sidecar with direct access to an injected payload source, initially a local Engine API; the BN keeps chain inputs, validation, and publication | See [the direct-Engine working plan](provisional-direct-engine-plan.md). A shared EL is acceptable for a PoC only when the Builder follows BN-emitted attributes exactly; production shared-versus-dedicated topology remains open |
...
> The `BN-02`, `BN-03`, and `BN-04` task blocks below are retained as historical BN-mediated design. Their active replacements are `PAYLOAD-01`, `STORE-01`, and `BID-CORE-01` in the [direct-Engine working plan](provisional-direct-engine-plan.md) and Linear. Do not implement the historical tasks as current instructions.
