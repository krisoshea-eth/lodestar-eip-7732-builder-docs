...
**Status:** v1.0 merged on 5 August 2026. All plan-review comments are resolved and no plan-level architecture question remains open. This execution copy was reconciled on 24 August against Lodestar v1.46.0, current `unstable` at `157b2b2a96`, Marco's merged Builder follow-ups through #9868, Marco's four open event PoCs, open BN API #9832 and SSE #9872, the refreshed API-02 fork draft, consensus-specs alpha.14, the merged Builder/Beacon/Keymanager API flow, and the Linear/GitHub backlog. The full plan remains canonical on GitHub because it exceeds HackMD's per-note character limit. Moving implementation details continue to be refined through their owning issues without reopening the accepted architecture.
