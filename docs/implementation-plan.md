...
**Working model:** small reviewable branches plus a temporary combined integration branch; inspect current `unstable` without routine branch-refresh merges; update dependencies only when needed
**Formal product name:** `lodestar builder` / `packages/builder`  
**Optional EPF mission codename:** Forgestar  

**SPEC-01:** [Beacon APIs draft #641](https://github.com/ethereum/beacon-APIs/pull/641) proposes extending `block`, with the lightweight `bid_included` alternative linked. LOD-48 is In Review and assigned to Kris. Cross-client selection and implementation evidence remain open; this does not settle payload-attributes #638.

**Status:** Reviewed through 13 September 2026. Nico's confirmed direct-Engine direction and the [direct-Engine working plan](direct-engine-working-plan.md) control conflicts with the historical BN-mediated sections below. API-02, TEST-01, the ledger, preference tracker, Marko's store and policy, and their contribution PRs have merged. Naming follow-up #10063 and API setup cancellation #10065 have also merged. Six PRs are ready for maintainer review: #9958, #10054, #9979, #9980, #9982 and shared events #10064. Ready is not approved for merge. #10054 still awaits Nazar's re-review. The [13 September reconciliation](reviews/2026-09-13-reconciliation.md) records exact-head hosted-check gaps, including #9979's checkpoint-sync timeout. #9973, #9978 and #9981 remain drafts dependent on open #9958. Fork #77 is resolved-input integration; fork #80 remains tied to the payload-attributes proposal. Existing replies are posted; no new unanswered comments were found in this refresh. LOD-76/77/78 are assigned to Kris for the input consumer, Builder bid wiring and reveal wiring.

The [input experiment](builder-input-contract.md) progresses LOD-76/77/78 on `krisoshea/input-consumer`, separately from fork #77's existing PR. It consumes typed events through a bounded head-only Gloas path, using existing `head_v2` roots and #80's provisional finality-hash transport. Explicit programmatic `bidRuntime` options let `Builder.init()` construct the bid services and connect the shared stream. At `523638b2f8`, it also connects exact selection and retained-envelope publication, requiring an explicit reveal policy and bounded cutoff. Its 198 targeted tests and Builder package checks pass. CLI/Engine transport construction, recovery, Heze inputs, reviewed reveal retry/settlement/eviction and a complete real BN/EL lifecycle remain unfinished. No accepted input specification or production topology is implied.

> **Working architecture change:** The accepted plan below remains as design history and for unaffected delivery rules. Do not implement its BN-owned payload construction or stateful reveal assumptions without first applying [the direct-Engine reconciliation](direct-engine-working-plan.md). The Builder talks directly to an injected payload source, initially a local Engine API; it owns payload material and bid construction, while the source BN retains chain authority, validation, and publication. Production shared-versus-dedicated EL topology remains open and does not block review of pure service boundaries.
...
| Core issue model | Original delivery issues plus focused Gate-A, direct-Engine, runtime, evidence, and specification children |
...
> **11 September review:** Merged PRs do not imply a complete runtime. New follow-ups cover one shared event stream (LOD-87), preference bootstrap (LOD-88), the `payloadStore` field name (LOD-89), real-node metric observations (LOD-91), and shared Engine types (LOD-92). Marko's existing LOD-86 is the canonical two-slot retention review; the duplicate LOD-90 is closed. #63's broader store guarantees remain unaccepted. SPEC-01 remains a cross-client comparison, and ENV-02 outreach remains paused.
...
This diagram describes the first Builder-owned store used by the direct-Engine implementation. The first reviewed loop may use bounded in-memory retention, which the team has accepted for the initial lifecycle. Durable recovery remains separate.
...
If the Builder process remains unavailable through the reveal window, or its in-memory payload material is lost after a bid is accepted and selected, the Builder may still owe the proposer while failing to reveal the payload. A source-BN outage can also prevent observation or publication. These are real protocol and operational failures, not something the first iteration must solve with redundancy.

The first iteration must:

- make this failure visible in logs and outcome evidence;
- never claim successful reveal when the source BN, Builder, or exact reveal material is unavailable;
- document the financial consequence;
- support only the bounded Builder-state and event recovery defined in `REL-01`;
- leave redundant instances, durable failover, and cross-node recovery to deferred work.
...
- BN-owned payload construction or an unreviewed second Engine orchestration path;
- remote signer or validator keymanager integration;
- multiple Builder keys;
- high availability, redundant Builder instances, multi-BN failover, or durable recovery beyond the bounded state/event path in `REL-01`;
...
|---|---|---|---|
| `D-01` | Base and PR model | Target `unstable` with focused reviewable PRs; use a temporary combined branch for integration evidence | Record exact dependency heads. Resolve actual conflicts with incremental commits; do not routinely merge `unstable` into every review branch |
...
| `D-03` | Runtime boundary | **Updated 2 September.** The working baseline is a standalone sidecar with direct access to an injected payload source, initially a local Engine API; the BN keeps chain inputs, validation, and publication | See [the direct-Engine working plan](direct-engine-working-plan.md). A shared EL is acceptable for a PoC only when the Builder follows BN-emitted attributes exactly; production shared-versus-dedicated topology remains open |
...
| `D-05` | Registration, fee recipient, and funds | Builder onboarding, top-ups, withdrawals, and balance management stay external. The Builder passes its configured execution fee recipient to the injected `PayloadSource` before preparing a payload; this does not require BN-mediated delivery. The address is Builder-controlled, need not match its withdrawal credentials, and is distinct from the proposer's bid payment address | Carry the configured execution fee recipient through `BuildRequest` to the Engine payload attributes. Obtain `bid.fee_recipient` separately from matching proposer preferences; preserve separate configuration and keep top-ups external |
...
| `D-08` | Solvency and balance visibility | The Builder computes a coverable bid against its current balance and pending obligations; the BN independently validates submitted bids | The first policy stays simple, exact-width, and bounded. It must not publish a bid whose retained payload or payment is not coverable; top-ups and durable restart recovery remain external/follow-up concerns |
| `D-09` | Source-BN trust | The BN is authoritative for proposer context, preferences, chain inputs, validation, publication, and outcomes; the Builder's payload source is authoritative for locally built payload material and value | Preserve typed boundaries between BN inputs, payload construction, storage, policy, and publication. Do not silently transfer BN custody identity or fork-choice ownership to the Builder |
| `D-10` | Bid timing and head view | Follow source-BN chain inputs, produce a bounded local candidate set, and submit at most one bid per slot and parent tuple through the flood-publication path merged in #9914 | Publication timing, parent-of-head variants, and non-finality breadth remain bounded policy decisions; per-call flood publication is no longer deferred |
...
| `D-13` | Reveal cache and stale work | Use the accepted in-memory store from #9970, with explicit block-hash keys, replacement and slot pruning. Hard capacity and first-write protection are not accepted guarantees | Store before publication and cancel stale build work. Reconcile retention with the reveal lifecycle under LOD-86; persistence and restart recovery follow after the honest in-memory loop works |
...
| [Lodestar #9736](https://github.com/ChainSafe/lodestar/pull/9736) | Open draft: correct FULL-parent state use for block production and reward calculation | Track it as BN/runtime evidence for `PAYLOAD-01`, `EXT-PREP-01`, and outcome testing; do not build a parallel FULL-parent path in the Builder sidecar |
...
| [Lodestar #9958](https://github.com/ChainSafe/lodestar/pull/9958) / [#9973](https://github.com/ChainSafe/lodestar/pull/9973) / [#9957](https://github.com/ChainSafe/lodestar/pull/9957) | #9958 is the ready PAYLOAD-SOURCE-01 extraction. Draft #9973 adds architecture-neutral bounded orchestration without runtime wiring. #9957 removes older pre-Fulu blob retrieval code but retains Gloas `getPayload` blob bundles | Review #9958 independently, keep #9973 stacked until the source boundary stabilizes, and keep #9957 as a baseline watch rather than a reason to remove blobs from `BuiltPayload` |
| Marko-owned [LOD-68](https://linear.app/kriso/issue/LOD-68/store-wiring-01-wire-and-prune-the-builder-payload-store), [Lodestar #9970](https://github.com/ChainSafe/lodestar/pull/9970), and [contribution #9](https://github.com/markolazic01/lodestar/pull/9) | #9970 merged, including #9 | Keep the initial store simple. Incorporated #9 adds pruning/lookup tests; copying, capacity and first-write guarantees from the older proposal remain outside accepted scope |
| Marko-owned [LOD-69](https://linear.app/kriso/issue/LOD-69/bid-policy-base-01-add-the-initial-builder-bid-policy), [Lodestar #9974](https://github.com/ChainSafe/lodestar/pull/9974), [contribution #10](https://github.com/markolazic01/lodestar/pull/10), [#9975](https://github.com/ChainSafe/lodestar/pull/9975), and [#9976](https://github.com/ChainSafe/lodestar/pull/9976) | Policy, ledger and preferences are merged, including #10's arithmetic contribution. Broader per-call validation remains open in LOD-64; winning liabilities remain until explicit settlement | Review the pure foundations before final `SlotBidder` wiring; keep exact-width arithmetic and dependent-root ownership explicit |
| [Lodestar #9978](https://github.com/ChainSafe/lodestar/pull/9978) / [#9979](https://github.com/ChainSafe/lodestar/pull/9979) / [#9980](https://github.com/ChainSafe/lodestar/pull/9980) / [#9981](https://github.com/ChainSafe/lodestar/pull/9981) / [#9982](https://github.com/ChainSafe/lodestar/pull/9982) | #9979/#9980/#9982 are ready for component review; #9978/#9981 remain drafts dependent on #9958. #9979 preserves Heze bid typing, #9981 binds the retained parent Beacon root, and #9982 separates reveal reservation from successful publication | Review readiness does not establish acceptance or runtime integration. Keep the bid and selection/reveal paths coherent while reviewing their component changes |
| [fork #77](https://github.com/krisoshea-eth/lodestar/pull/77) / [LOD-73](https://linear.app/kriso/issue/LOD-73/slot-bidder-01-coordinate-one-resolved-direct-engine-bid) | Fork-only stacked draft composes one resolved input through payload build, retention, coverability, bid assembly, and publication | Review as integration evidence. Keep event consumption, Builder/CLI construction, EL configuration, selection, and reveal outside this draft |
| [fork #80](https://github.com/krisoshea-eth/lodestar/pull/80) / [LOD-74](https://linear.app/kriso/issue/LOD-74/attr-impl-01-emit-post-gloas-forkchoice-hashes-in-payload-attributes) | Fork-only draft adds the proposed post-Gloas safe/finalized execution hashes to Lodestar's event type and current producer | Keep it fork-only while #638 is reviewed. [LOD-75](https://linear.app/kriso/issue/LOD-75/attr-emit-01-emit-deduplicated-post-gloas-payload-attributes) owns trigger, FULL/EMPTY policy, deduplication, and custody behavior; [LOD-76](https://linear.app/kriso/issue/LOD-76/attr-consume-01-consume-fork-correlated-payload-attributes-in-builder) owns Builder consumption |
| [consensus-specs #5594](https://github.com/ethereum/consensus-specs/pull/5594) / [Lodestar #9972](https://github.com/ChainSafe/lodestar/pull/9972) | Merged Lodestar validation rejects a bid whose block hash equals its parent hash | BN validation remains authoritative. The existing #9978/#77 local pre-sign guard is complementary fail-fast coverage of the same invariant |
| [Lodestar #9998](https://github.com/ChainSafe/lodestar/pull/9998) / [LOD-79](https://linear.app/kriso/issue/LOD-79/bid-publish-api-01-remove-bid-pool-addition-in-api-path) | Merged API path validates and flood-publishes a bid without inserting it into the receiving BN's local pool | Use a separate proposer BN that receives the bid over p2p for honest-loop selection evidence; do not treat a self-build on the submission BN as publication failure |
| [Lodestar #9994](https://github.com/ChainSafe/lodestar/pull/9994) | Closed unmerged; the proposal is not delivered source behavior | Track its final disposition in REL-01 and QA-01 while keeping unknown post-Gloas roots eligible for recovery |
| [Lodestar #9984](https://github.com/ChainSafe/lodestar/pull/9984) / [#9986](https://github.com/ChainSafe/lodestar/pull/9986) | Merged BN bid-validation ordering and merged shared spec-test expected-error enforcement; #9986 exposes Gloas Builder and validator sweep-index divergences | Reuse the BN validation order and route refreshed invalid-vector evidence to QA-01; neither change creates another standalone Builder service |
| [Buildoor #184](https://github.com/ethpandaops/buildoor/pull/184) | Merged: accepts Builder preferences without an external Builder URL and exercises local/p2p Builder selection paths | Reuse for E2E-01; preserve `execution_payment = 0` for p2p bids and keep its no-URL integration coverage limitation explicit |
| [consensus-specs beta.0](https://github.com/ethereum/consensus-specs/releases/tag/v1.7.0-beta.0) / [Lodestar #9955](https://github.com/ChainSafe/lodestar/pull/9955) | Released September 3 and adopted by Lodestar September 4; includes parent-header slot semantics and PTC type-name changes | Keep BASELINE-01's alpha.14 record immutable. Qualify current runtime inputs against beta.0 separately; no automatic PR branch refresh or new `PayloadSource` abstraction is required |
| [Buildoor #186](https://github.com/ethpandaops/buildoor/pull/186) | Open geth-only deterministic testing source using `testing_buildBlockV1`, with exact transaction-plan verification | Optional E2E-01/INT-01 evidence only; do not replace the production Engine boundary or require a non-standard testing namespace |
| [ethereum-package #1483](https://github.com/ethpandaops/ethereum-package/pull/1483) | Open fix assigning genesis-registered Builder keys to launched Buildoor instances and documenting their activation delay | Pin its disposition for E2E-01/INT-01 before interpreting missing Buildoor bids; this does not change Lodestar Builder code ownership |
| [consensus-specs #5573](https://github.com/ethereum/consensus-specs/pull/5573) / [#5590](https://github.com/ethereum/consensus-specs/pull/5590) | Open draft compliance generator and non-normative envelope ReqResp format exploration | Reuse accepted vectors in QA/OUT/REL work; do not create another Builder service or assume the exploratory transport format is normative |
| [execution-apis #870](https://github.com/ethereum/execution-apis/pull/870) / [#878](https://github.com/ethereum/execution-apis/pull/878) | Open Heze inclusion-list response-bound clarifications | Track under conditional EXT-FOCIL-01; no change to the current Gloas implementation path |
...
| [Lodestar #9937](https://github.com/ChainSafe/lodestar/pull/9937) / [#9994](https://github.com/ChainSafe/lodestar/pull/9994) / [#9791](https://github.com/ChainSafe/lodestar/pull/9791) / [#9326](https://github.com/ChainSafe/lodestar/pull/9326) | #9937 merged; #9994 and #9281 closed unmerged; the other BN recovery changes remain watches | Add bounded EMPTY, impossible-target, stale-root, and non-finality cases to REL-01 and E2E evidence without creating another Builder service |
| [Lodestar v1.47.0](https://github.com/ChainSafe/lodestar/releases/tag/v1.47.0) / [#9971](https://github.com/ChainSafe/lodestar/pull/9971) | Latest stable at `450996b13a`, published 2 September and merged back to `unstable` through #9971 | Newest immutable release target. It includes the earlier Builder foundation but not the current direct-Engine service PR series or its integrated runtime |
| [Lodestar v1.46.0](https://github.com/ChainSafe/lodestar/releases/tag/v1.46.0) | Historical stable release at `3873dd5b032d0ad82581fc3416e9628b4f6f2642`, published 12 August. It predates merged #9781 | Retain as immutable historical evidence; v1.47.0 is the current release target |
| [Lodestar v1.47.0-rc.0](https://github.com/ChainSafe/lodestar/releases/tag/v1.47.0-rc.0) | Historical prerelease at `2aff495`, published 27 August, containing #9832, #9781, #9848, and the surrounding Gloas Builder API foundation | Superseded by v1.47.0; retain only as prerelease qualification history |
| [Lodestar v1.46.0-rc.1](https://github.com/ChainSafe/lodestar/releases/tag/v1.46.0-rc.1), [#9790](https://github.com/ChainSafe/lodestar/pull/9790), [#9792](https://github.com/ChainSafe/lodestar/pull/9792), and [#9793](https://github.com/ChainSafe/lodestar/pull/9793) | Historical release-candidate baseline superseded by later stable releases. #9790 protects state persistence/database close during a stuck network-worker shutdown and #9792 fixes a QUIC resource leak. #9793 closed without merge because its self-signal/force-exit approach did not generalize, especially for default container PID 1. The underlying stuck handle remains unidentified | Retain as diagnostic history for state persistence, process-manager timeout, and Builder restart/cache/reveal recovery; do not adopt the rejected self-signal approach |

The board preserves completed foundation work. API-02, TEST-01, BID-LEDGER-01, PREF-01, STORE-WIRING-01 and BID-POLICY-BASE-01 are **Done** for their merged scopes. PAYLOAD-SOURCE-01, BID-PUBLISH-01, SELECT-MATCH-01 and REV-PUBLISH-01 are **In Review**; dependent drafts remain in progress. LOD-89 is Done through #10063. ENV-02 remains In Review with outreach paused. LOD-87 is In Review through #10064. API cancellation #10065 is merged, but broader REL-01 recovery remains incomplete. LOD-76/77/78 are In Progress and assigned to Kris for the fork-only input, startup and reveal work. Preference recovery LOD-88 remains unassigned in Backlog. LOD-93 tracks Kris's existing simulation PR #10054 as an In Review child of QA-01. New head-freshness and benchmark watches extend BN-01 and QA-01, not duplicate component issues. The current pass is a targeted reconciliation, not a new whole-board audit.
...
| **Gate B — Real bid path** | End W12 | Builder constructs one complete coverable bid from a direct-Engine payload, retains exact reveal material, and submits it through the BN validation/publication path |
...
| **Gate D — Protocol-complete local evidence** | End W16 | PTC/payment evidence, non-zero blobs/data, essential diagnostics, and bounded Builder-state/event recovery are complete |
...
ENV-01 / CLI-01
→ SIGN-01 / API-01
→ API-02 / BN-01 / EL-ARCH-01
→ PAYLOAD-SOURCE-01 → PAYLOAD-ORCH-01
→ STORE-01 → BID-CORE-01 → BID-01
→ SELECT-01 → REV-01
→ E2E-01
→ OUT-01 / DATA-01
→ QA-01 / REL-01
...
**Current disposition, 11 September:** [Lodestar #9931](https://github.com/ChainSafe/lodestar/pull/9931) has merged and API-02 is Done for its implementation scope. Existing real-BN event and shutdown evidence remains historical; independent reproduction stays in ENV-02 and does not reopen the merged observer. Separate test-only [#10054](https://github.com/ChainSafe/lodestar/pull/10054) at `cbdbf682cda6` passed a fresh local multifork simulation after Nazar's simplification requests. That simulation is not a complete Gloas Builder lifecycle run.

**Operational handoff:** Node 24.13.0 uses Lodestar's npm `eventsource` fallback. The current Builder CLI configures one source BN, while the shared API client can otherwise serve REST calls from fallback URLs and pins SSE to its first URL. The Beacon API event contract defines no SSE `id` or `Last-Event-ID` resumption, so conforming clients cannot be assumed to replay a missed selection notification. Merged #9872 and #9964 keep the connection or event loop alive after individual serialization, decode, or consumer failures, which makes connected-stream reconciliation necessary as well as reconnect recovery. `ENV-02` owns real-process shutdown and independent-reproduction evidence; `REL-01` owns bounded same-source reconciliation after reconnect and across connected-stream gaps; deferred multi-BN and long-gap behavior remain in their existing tracking issues.
...
**Board status:** Done through merged [Lodestar #9932](https://github.com/ChainSafe/lodestar/pull/9932) on 9 September. The original task inventory below is retained as historical planning context; [LOD-45](https://linear.app/kriso/issue/LOD-45) records the reconciled delivered scope and evidence.

**Original task inventory (historical)**
...
> The `BN-02`, `BN-03`, and `BN-04` task blocks below are retained as historical BN-mediated design. Their active replacements are `PAYLOAD-01`, `STORE-01`, and `BID-CORE-01` in the [direct-Engine working plan](direct-engine-working-plan.md) and Linear. Do not implement the historical tasks as current instructions.
...
| ID | Individual issue | Lane | Target | Effort | Ready after |
|---|---|---|---:|---:|---|
| `BID-01` | Sign and submit retained Builder bids | Builder | W12 | M | `SIGN-01`, `API-01`, `STORE-01`, `BID-CORE-01`, `BN-PUB-01` |
...
flowchart TD
    ...
    OBSERVE -->|Builder restarted or event missed| RECOVER["REL-01 bounded Builder state and event recovery"]
    RECOVER --> MATCHQ
```
...
- [ ] Pass the selecting block root and exact selected-bid identity to the stateless reveal flow backed by the retained local payload record.
...
**Done when:** Seeing a valid block containing the local bid starts one idempotent stateless reveal workflow for that block root and exact retained payload identity.
...
**Why:** The EL may include blobs even in the first working loop, and the Builder must retain their exact reveal material while the BN handles publication and data availability. The final core additionally forces a non-zero-blob case so stateless reveal proves real blob/data commitments.

**Tasks**

- [ ] Produce at least one candidate containing non-zero blobs.
- [ ] Verify commitments and required blob/KZG/data-column source material survive payload → bid → Builder-owned stateless reveal.
- [ ] Publish the stateless envelope and its retained blobs/KZG proofs through the source BN, then allow the BN to gossip the resulting data.
...
- [ ] Keep cross-BN recovery and high availability conditional; the first p2p bid-selection fixture may still use separate submission and proposer BNs.

**Done when:** A selected non-zero-blob bid reveals from exact Builder-retained material and reaches authoritative FULL/data-available evidence.
...
#### `REL-01` — Add bounded Builder restart and event recovery

**Why:** The first direct-Engine loop keeps signed bids, payload material, and ledger state in memory. Restart and missed-event behavior must therefore be explicit and fail closed until a reviewed durable record exists. This is bounded recovery, not high availability.

**Tasks**

- [ ] Reconnect to the same verified source BN and resume block-event consumption after a Builder restart.
- [ ] Define the minimum durable signed-bid, payload-store, and ledger record required to recover an exact stateless reveal; until it exists, report restart loss as a precise terminal outcome.
- [ ] Preserve source affinity between SSE, block retrieval, and publication operations unless a reviewed cross-BN trust contract exists.
- [ ] Reconcile a bounded missed-event window after reconnect and while the stream remains connected. The Beacon API event contract defines no SSE `id` or `Last-Event-ID` resumption, and merged Lodestar #9872/#9964 can preserve the connection while dropping one failed event.
- [ ] Deduplicate replayed roots and publication attempts. Preserve the exact reveal reservation across an ambiguous attempt and retry only identical envelope material within the runtime cutoff.
- [ ] Track open [Lodestar #9994](https://github.com/ChainSafe/lodestar/pull/9994) for impossible known genesis/pre-Gloas envelope-sync targets while keeping unknown post-Gloas roots recoverable.
- [ ] Never reconstruct a replacement payload after selection.
- [ ] Add restart-before-selection, restart-after-bid, bounded gap reconciliation, duplicate publication, wrong signature, missing durable material, source-affinity rejection, and shutdown tests.

**Done when:** The supported restart window and durable state are explicit and tested; recoverable exact material can still reveal, while missing state fails closed with a precise outcome.
...
- [ ] Repeat source-BN inputs → direct-Engine payload → retained coverable bid → p2p selection → immediate stateless reveal → FULL.
- [ ] Use at least two Beacon Nodes after merged [Lodestar #9998](https://github.com/ChainSafe/lodestar/pull/9998): one accepts and flood-publishes the API bid, while a separate proposer BN receives it over p2p and selects it.
- [ ] Assert that the submission BN does not select its API-only bid from a local pool and that its self-build is not reported as a publication failure.
...
- [ ] Inspect current `unstable`, incorporate only required compatibility changes, rerun critical evidence, and resolve or ticket findings.
...
2. Inspect current `unstable` regularly, but do not refresh review branches routinely. Resolve actual conflicts or compatibility changes with incremental commits.
...
- [x] Create and reconcile the current Linear issue set, including the original core, Gate-A follow-ups, direct-Engine service slices, environment follow-up, cross-repository specification tracks, contributor-attribution records, payload-attributes producer/consumer work, and the two runtime-consumer boundaries. Keep canceled duplicates and GitHub-only administrative history out of active-work counts.
