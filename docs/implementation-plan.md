...
- [x] Create and reconcile the 70 current Linear issues, including the original core, Gate-A follow-ups, direct-Engine service slices, environment follow-up, cross-repository specification tracks, and contributor-attribution records. The GitHub repository has 72 issues because closed legacy `NICO-01` and the unlabelled administrative `PRESENTATION-01` are not Linear project items.
