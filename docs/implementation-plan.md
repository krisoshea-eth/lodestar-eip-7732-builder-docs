...
**Status:** v1.0 merged on 5 August 2026. Nico confirmed direct Engine access as the working direction for `packages/builder` on 2 September after the project audited his 10-commit proof-of-concept branch. The [direct-Engine working plan](direct-engine-working-plan.md) controls any conflict with the historical BN-mediated payload, bid, cache, and reveal sections below. The 3 September Lodestar `unstable` snapshot was `76b167bf36`. #9832, #9904, #9914, #9964, #9972, #9984, #9986, #9998, and js-libp2p #3610 have merged. #9958 is ready for review; #9973 and the bid/reveal service drafts expose later review boundaries; #9970/#9974 plus Kris's contribution PRs are the active store and policy paths. Fork draft #77 exercises the resolved-input bid services without deciding event or CLI wiring; the 7 September audit's missing post-policy coverability guard is fixed and tested at `75037946b79e`. Fork draft #80 implements the current beacon-APIs #638 safe/finalized hash shape while leaving payload-attributes trigger and deduplication open. Unassigned BID-RUNTIME-01 and REV-RUNTIME-01 own the remaining production-consumer boundaries. API-02, TEST-01, ENV-02, and SPEC-01 retain their independent scopes.
...
The board preserves `CLI-01`, `API-01`, `SIGN-01`, `MET-01`, `BN-PUB-01`, and `BASELINE-01` as **Done**. `ENV-01` is Done for its manual development-environment scope. `ENV-02` owns clean-checkout automation and remains **In review** pending independent reproduction. `REVIEW-01` remains **In progress** only for explicit #9781 thread-marker reconciliation, while TEST-01 is **Done** and `API-02`, PAYLOAD-SOURCE-01, PREF-01, and STORE-CORE-01 are **In review**. BN-01, SPEC-01, EL-ARCH-01, PAYLOAD-01, STORE-01, BID-CORE-01, BID-01, SELECT-01, and REV-01 remain **In progress**. Epic C is In Progress because its bid, selection, and reveal child work is active. Landed prerequisites and pure helpers narrow a consuming issue but do not automatically complete its integrated outcome.
...
**Current evidence, 9 September:** Upstream [Lodestar #9931](https://github.com/ChainSafe/lodestar/pull/9931) is the review artifact at head `c135ae8cc953`. The shared stub conflict with merged TEST-01 was resolved without an `unstable` merge. All 48 relevant Builder tests, package type-check, changed-file Biome and `git diff --check` pass on Node 24.13.0. The earlier multifork simulation failure was traced to an already-imported block returning success where the test expected rejection; a corrected simulation and fresh CI result remain outstanding. The earlier real-BN event-to-block and connected/interrupted-stream shutdown evidence remains stored under ENV-02; issue closure still requires its independent reproduction.
