...
- [x] Create and reconcile the 72 current Linear issues, including the original core, Gate-A follow-ups, direct-Engine service slices, environment follow-up, cross-repository specification tracks, contributor-attribution records, SLOT-BIDDER-01, and ATTR-IMPL-01. The GitHub repository has 74 issues because closed legacy `NICO-01` and the unlabelled administrative `PRESENTATION-01` are not Linear project items.
