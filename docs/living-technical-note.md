...
| Builder implementation | Initial [`@lodestar/builder`](https://www.npmjs.com/package/@lodestar/builder) package merged in [#9758](https://github.com/ChainSafe/lodestar/pull/9758); [#9781](https://github.com/ChainSafe/lodestar/pull/9781) is an open review-required draft at `251347b7b5` with 33 commits and eight unresolved Nico threads; `SIGN-01`, `CLI-01`, and `API-01` are Done by project decision, `REVIEW-01` and `TEST-01` are In Progress, API-02 is In Review, and `MET-01` is Todo |
| Devnet | [glamsterdam-devnet-7](https://notes.ethereum.org/@ethpandaops/glamsterdam-devnet-7) remains the active published configuration on alpha.12/v7.2.0. Four Tysm/Prysm host definitions were removed in devnet commit [`1ca063f`](https://github.com/ethpandaops/ethereum-package/commit/1ca063f64d17df4479926d380f527bc2181b3f07), and the Dora image override changed in [`df1dfc7`](https://github.com/ethpandaops/ethereum-package/commit/df1dfc727b278eddfbf93f5bb42c83ee5f52e2ff); these are configuration evidence, not runtime-health proof. [`tests-glamsterdam-devnet@v8.1.0`](https://github.com/ethereum/execution-specs/releases/tag/tests-glamsterdam-devnet%40v8.1.0) is the latest successor fixture release but does not establish a devnet-8 launch |
...
| Last reconciliation | August 11, 2026: direct Lodestar and project-board checks, Builder PR #9781 review state, API-02 implementation and tests, implementation plan, Linear, and GitHub issue mirror |
| Next milestone | Resolve and merge #9781, reconcile its final Builder initialization with API-02, finish `BASELINE-01` and `ENV-01`, and continue the separated `TEST-01` work before activating the reviewed BN preparation/candidate design while `MET-01` proceeds separately |
...
| 2026-08-10 | Later-deposited Builder lifecycle | **Direction reviewed; implementation open** | When the configured key is absent from the BN result, keep the sidecar inert and retry with cancellation so a later deposit or activation can be discovered. Keep a returned non-active lifecycle status distinct and operator-visible. `REVIEW-01` owns implementation and merge; `TEST-01` owns the regressions |
...
| 2026-08-09 | Builder Gate-A issue split | **Completed scopes and follow-ups remain separate** | Marko's `CLI-01` and `API-01` closure is preserved. `REVIEW-01` owns the open #9781 review/lifecycle implementation/merge work, while `TEST-01` and `MET-01` own the remaining test matrix and metrics |
...
These items were reconciled against the August 10 partial monitoring report, confirmed Lodestar-team guidance, and direct live primary-source checks through August 11. The partial run did not advance unavailable source cursors or establish runtime health. Re-check them after each baseline bump.
...
- `SIGN-01`, `CLI-01`, and `API-01` are complete in the project board. `REVIEW-01` keeps the open [Lodestar #9781](https://github.com/ChainSafe/lodestar/pull/9781) review, lifecycle implementation, documentation, and merge work visible; `TEST-01` and `MET-01` own the remaining cross-cutting test and metric work.
- [Lodestar #9781](https://github.com/ChainSafe/lodestar/pull/9781) is an open review-required draft at head `251347b7b5c5723a84ca6851e603604cdc75cd80`, with 33 commits and eight unresolved Nico review threads on August 11. It adds Builder identity/status tracking, BN and EL readiness polling, `--executionFeeRecipient`, request timeout wiring, and identity/tracker tests. The branch uses the API client's standard `.value()` response path and asynchronous API-error response mocks. The reviewed later-Builder direction is to keep an unknown configured key inert and retry with cancellation until a later deposit or activation can be discovered, while treating a returned non-active lifecycle status as a distinct operator-visible result. Remaining review work covers the later-Builder lifecycle, bounded useful error detail, and the current assumption that Gloas-capable clients expose the health endpoint.
- The current #9781 tests cover successful Builder-index resolution, returned non-active status, version mismatch, status-lookup failure, initial tracker state, first successful polling, balance-only changes, lifecycle-status changes, and failed-poll state preservation. Unknown-key polling and later activation, cancellation, readiness recovery, bounded error detail, genesis logging, and CLI cases remain in `TEST-01`.
...
- [#9788](https://github.com/ChainSafe/lodestar/pull/9788) is the open draft v1.46.0 release PR at the rc.1 commit. It is a release watch, not evidence that v1.46.0 stable has shipped.
- [#9780](https://github.com/ChainSafe/lodestar/pull/9780) is an open circuit-breaker follow-up outside rc.1. Circuit-breaker thresholds and recovery remain unsettled and do not change the core Builder sidecar scope.
...
Important moving inputs on August 11:

- [#9781](https://github.com/ChainSafe/lodestar/pull/9781), open, review-required, and draft at `251347b7b5`: Gate-A Builder identity, readiness, configuration, lifecycle implementation, and tests;
...
- [#9788](https://github.com/ChainSafe/lodestar/pull/9788), open draft: v1.46.0 release preparation at the rc.1 commit, not a stable release;
- [#9780](https://github.com/ChainSafe/lodestar/pull/9780), open: circuit-breaker review follow-up outside rc.1, with policy still unsettled;
...
- **Source-BN readiness:** resolve and merge [#9781](https://github.com/ChainSafe/lodestar/pull/9781), including the reviewed unknown-key inert wait/retry implementation, standard API-client response handling, bounded useful error detail, and the remaining health-endpoint thread; close the separated regressions through `TEST-01`.
...
Status reflects the August 11 reconciliation. The merged implementation plan remains the delivery source of truth; this map highlights moving code seams and upstream work.

| Area | File / PR | Current understanding | Builder follow-up |
|---|---|---|---|
| Builder package and CLI | [#9758](https://github.com/ChainSafe/lodestar/pull/9758), [#9766](https://github.com/ChainSafe/lodestar/pull/9766), [#9781](https://github.com/ChainSafe/lodestar/pull/9781) | Initial package and TypeScript follow-up are merged; #9781 is an open review-required draft at `251347b7b5` with identity/status tracking, readiness, fee-recipient and timeout wiring | Keep CLI-01/API-01 closed by project decision; resolve #9781 feedback, lifecycle implementation, documentation, and merge in REVIEW-01 while TEST-01/MET-01 continue separately; keep generated CLI docs hidden until functional |
...
| Source-BN client and readiness | [#9781](https://github.com/ChainSafe/lodestar/pull/9781), BN sync helpers | Open draft at `251347b7b5` with eight unresolved Nico threads. Standard `.value()` response handling and asynchronous error-response mocks match the reviewed direction; identity/tracker tests cover the main returned-state and polling cases, while unknown-key polling/later activation, cancellation, readiness recovery, and bounded error detail remain follow-up | Implement the reviewed inert wait/retry path for an unknown configured key, preserve returned non-active status separately, retain the BN-owned preparation guard, and finish the missing test matrix in TEST-01 |
...
Lodestar serializes `event` and `data` fields but no SSE `id`. Automatic EventSource reconnect can resume transport, but `Last-Event-ID` cannot identify or replay notifications missed during the gap. `REL-01` must perform explicit bounded same-source reconciliation, while long disconnects, deeper reorgs, and competing roots remain in deferred LOD-41. `ENV-01` must also prove that a real Builder process exits cleanly on `SIGTERM` both with a connected stream and during reconnect.
...
| `CLI-01`, `API-01` | Done | Closure preserved in line with Marko's project-status decision; REVIEW-01 carries the remaining #9781 review, lifecycle implementation, documentation, and merge work |
| `REVIEW-01` | In progress | Resolve eight #9781 review threads, implement the reviewed unknown-key inert wait/retry behavior, document responsibility boundaries, and record final merge evidence |
...
| `API-02` | In review | Draft implementation PR #48 and docs PR #13 contain the audited bounded event path; #9781 integration and `ENV-01` end-to-end evidence remain before closure |
...
Status checked August 11, 2026 against live primary sources. The August 10 monitor was partial, so its unavailable source cursors were not advanced and no runtime-health inference was made. The tables prioritise items that can change the Builder architecture or current baseline; the daily monitor remains the broader watch inventory.
...
| [#9781](https://github.com/ChainSafe/lodestar/pull/9781) — Builder identity/readiness/CLI | Open review-required draft at `251347b7b5`; 33 commits and eight unresolved Nico threads. The standard `.value()` response path and async error mocks are in the branch | CLI-01/API-01 remain closed by project decision. REVIEW-01 owns the reviewed unknown-key lifecycle implementation and remaining review/merge work; TEST-01 and MET-01 retain their separated scopes |
| [#9788](https://github.com/ChainSafe/lodestar/pull/9788) — v1.46.0 release | Open draft at `e2b315e` | Release watch only; rc.1 remains the newest immutable audit target until a stable release lands |
| [#9780](https://github.com/ChainSafe/lodestar/pull/9780) — circuit-breaker follow-up | Open outside rc.1 | Policy and recovery remain unsettled; watch for baseline impact without adding a new core Builder workstream |
