...
| Architecture reconciliation | [Direct-Engine working plan](direct-engine-working-plan.md), confirmed direction with the PR/board audit refreshed on 13 September 2026; supersedes the historical BN-mediated payload and reveal design while production EL topology remains open |
...
| Builder implementation | API-02 #9931, TEST-01 #9932, store #9970, policy #9974, ledger #9975 and preference subscription #9976 are merged, alongside the earlier foundation and BN APIs. #9958 remains under review; the bid/reveal service drafts and runtime consumers remain unfinished. Nico's PoC remains implementation evidence, not a wholesale upstream merge |
| Devnet | Public [Platåberget Dora](https://dora.plataberget.ethpandaops.io/) supplied historical point-in-time evidence at [slot 79322](https://dora.plataberget.ethpandaops.io/slot/0x159ad62fd9512d3843f53ab79387a726d82b66fb0892134504cd1b426cc78b19). No fresh network-wide recovery, payload/reveal health or independent reproduction is established by this audit. [`tests-glamsterdam-devnet@v8.1.4`](https://github.com/ethereum/execution-specs/releases/tag/tests-glamsterdam-devnet%40v8.1.4) is the current fixture reference, published 3 September; publication does not prove deployment |
...
| Last reconciliation | September 13, 2026: [current reconciliation](reviews/2026-09-13-reconciliation.md), merged naming cleanup, review follow-ups, fork integration checks and new source-BN dependency watches. No complete Gloas Builder lifecycle or fresh devnet health is implied |
...
Later that day, the fork-only `krisoshea/input-consumer` experiment began LOD-76 implementation on top of fork #77. It correlates typed head/preference/payload events using the existing `head_v2` roots, keeps Builder execution proceeds separate from proposer bid payments, and uses the explicit #80 hash proposal. The [input-contract checkpoint](builder-input-contract.md) records its head-only Gloas limits. It is not yet connected to the running Builder dispatcher or CLI. LOD-77/78 remain unfinished, and no complete BN/EL lifecycle is claimed. Nico's new draft [#10070](https://github.com/ChainSafe/lodestar/pull/10070) adds a PTC gossip-arrival watch, not another Builder selection-event requirement.

The [12 September reconciliation](reviews/2026-09-12-reconciliation.md) supersedes moving statuses in the snapshots below. Shared events #10064 and API cancellation #10065 are now ready for review. #10054's hosted Tests/Sim checks passed; its Docs job failed during dependency installation. No whole-client or complete Gloas lifecycle result follows from those checks. API-02, the ledger, preference subscription, Marko's store/policy and the envelope metrics have merged. #9979/#9980/#9982 are now ready for component review after critical re-review and exact-head validation. Their inherited ledger files match upstream; descriptions separate that historical diff from their own review surface. #9973/#9978/#9981 still depend on open #9958. The fork-correlated request type in #9958 was retained after testing the suggested simplification. #10054 changes only the existing simulation helper and passed a full local Docker multifork run earlier on 11 September; the later critical re-review also passed nine local probes without reintroducing a test-utility suite into the PR. Neither validates the complete Gloas bid/selection/reveal lifecycle.
...
Follow-ups are recorded in LOD-87/88/89/91/92 and Marko's existing LOD-86. LOD-89 is Done through merged [#10063](https://github.com/ChainSafe/lodestar/pull/10063), a naming-only cleanup. The [13 September reconciliation](reviews/2026-09-13-reconciliation.md) supersedes older PR status snapshots. LOD-76/77/78 remain unassigned pending the agreed runtime responsibility split. SPEC-01's alternatives remain unpublished; Beacon APIs #638 is a separate, open payload-attributes proposal. ENV-02 outreach is paused.

## 10 September review update (historical)

The [current reconciliation](reviews/2026-09-10-reconciliation.md) supersedes moving status claims in the older snapshots. TEST-01 #9932 and ENV-03 #10010 are merged; store contribution #9 is incorporated; #9974 fixes fractional/non-finite shareBps; and amended #9976 includes preference subscription, pruning and shared-signal shutdown. Remaining service stacks stay draft. The separate simulation correction #10054 passed its full local multifork run and is ready for review. That Electra run is not proof of the Gloas Builder bid/reveal lifecycle. Fixtures [v8.1.3](https://github.com/ethereum/execution-specs/releases/tag/tests-glamsterdam-devnet%40v8.1.3) remain the fixture reference; neither fixture publication nor the historical Dora observation establishes current deployment health. Real Builder BN/EL integration and independent ENV-02 reproduction remain incomplete; ENV-02 outreach is paused.
...
|---|---|---|---|
| 2026-09-03 | API-submitted bid pool ownership | **Validate and flood-publish without local-pool insertion** | Merged [Lodestar #9998](https://github.com/ChainSafe/lodestar/pull/9998) removes API-submitted bids from the receiving BN's local pool. The honest E2E selection path now uses a separate proposer BN that receives the bid over p2p; local self-build on the submission BN is not a publication failure |
| 2026-09-03 | Component safety boundaries, corrected 11 September | **Preserve exact identity and retry state before runtime wiring** | #9973 rejects conflicting reuse of a job ID; merged #9976 retains received objects directly without a copying or read-only guarantee; accepted #9970 contribution #9 adds tests only, not material isolation; #9981 verifies the retained parent Beacon root; #9975/#9982 separate exact reveal reservation from successful publication; #9979 preserves Heze bid fields |
| 2026-09-03 | Parent execution hash invariant | **Reuse the merged BN validation** | [Lodestar #9972](https://github.com/ChainSafe/lodestar/pull/9972) merged the `block_hash != parent_block_hash` guard. BID-CORE-01 and QA-01 consume that behavior and fixtures rather than duplicating it in `packages/builder` |
...
| 2026-09-02 | Direct-Engine PR delivery | **Review foundations first and group consumers if maintainers prefer** | #9958, #9970/#9, #9974/#10, #9975, and #9976 establish foundations. #9973 remains stacked on #9958. Draft #9978/#9979 form one logical bid path; #9980/#9981/#9982 form one logical selection/reveal path. Fork draft #77 composes the resolved bid path. Unassigned BID-RUNTIME-01 and REV-RUNTIME-01 own the final bid and reveal consumers. Draft status records implementation evidence, not maintainer acceptance |
...
- Merged Lodestar #9914 and js-libp2p #3610 provide the current local-bid validation and flood-publication seam. Merged #9998 keeps API submissions out of the receiving BN's local pool, so the E2E selection fixture needs a separate proposer BN receiving the bid over p2p. Merged #9904 remains BN-side envelope import and recovery evidence rather than the Builder's primary direct-Engine payload store.
- Merged consensus-specs [#5549](https://github.com/ethereum/consensus-specs/pull/5549) adds post-Gloas `custody_columns` to `notify_forkchoice_updated`. Nico's proof of concept predates this input. BN-01, ATTR-01, PAYLOAD-01, and EL-ARCH-01 must settle the Builder node identity and authoritative source of this value before production runtime wiring. The injected payload-source contract can be reviewed independently.
- Ready [Lodestar #9958](https://github.com/ChainSafe/lodestar/pull/9958) extracts the narrow `PayloadSource` contract and injected Engine adapter as PAYLOAD-SOURCE-01. It does not settle production EL ownership or add Builder runtime construction. Draft [#9973](https://github.com/ChainSafe/lodestar/pull/9973) adds bounded architecture-neutral orchestration on top and now rejects conflicting reuse of one job ID. Draft [#9957](https://github.com/ChainSafe/lodestar/pull/9957) removes older pre-Fulu blob retrieval code but leaves the Gloas `getPayload` blob bundle used by this contract intact.
...
- [#9937](https://github.com/ChainSafe/lodestar/pull/9937) landed the EMPTY Gloas payload range-sync guard. Open [#9994](https://github.com/ChainSafe/lodestar/pull/9994), [#9791](https://github.com/ChainSafe/lodestar/pull/9791), and [#9326](https://github.com/ChainSafe/lodestar/pull/9326) remain related BN recovery watches for impossible known genesis/pre-Gloas envelope targets, stale payload roots, and non-finality cache retention. #9994 supersedes the conflicting #9281 implementation.
...
- define the authoritative safe/finalized hash, parent, proposer-preference, and custody-column inputs;
- establish payload-attributes trigger and deduplication semantics separately from the field-only implementation;
- enforce freshness and cancellation when inputs change, including a head change between an event and an Engine call;
- qualify shared EL use only as a PoC assumption: following BN events does not by itself prove that concurrent or stale `forkchoiceUpdated` calls cannot conflict;
- settle production Engine ownership while preserving support for a dedicated EL or alternative building software;
- keep the first working loop p2p-first, with in-memory retention accepted and Builder API server support and durable recovery deferred.
...
- Gate new bid work on accepted source-BN readiness and non-optimistic inputs, plus availability of the Builder's configured payload source. Runtime integration must enforce these checks; the BN retains its own authoritative publication validation. Do not rely on a BN preparation/candidate route that the direct-Engine design no longer proposes.
...
- **EL-invalid and recovery behavior:** open [#9332](https://github.com/ChainSafe/lodestar/pull/9332) and [#9637](https://github.com/ChainSafe/lodestar/pull/9637) feed EL-ARCH-01, QA-01, E2E-01, and OUT-01. Merged #9937 plus open #9994, #9791, and #9326 feed REL-01 and the bounded negative-path evidence; #9994 supersedes the conflicting #9281 implementation.
...
Nico confirmed direct Engine access as the working direction, in-memory retention for the first loop, and p2p-first delivery. Production EL sharing and exact runtime inputs still need implementation-level decisions. The following checks do not reopen the rejected BN-mediated baseline.

### Source-BN inputs and direct-Engine preparation

- Trace the BN's payload-attributes producer and fork-choice hash sources; use its existing Engine path as reference, not as a new Builder-only preparation API.
- Construct a fork-correct `BuildRequest` from authoritative inputs and the Builder-controlled execution fee recipient. Keep proposer payment preferences distinct from the payload coinbase.
- Specify freshness, FULL/EMPTY handling, deduplication, and custody-column ownership before wiring events into the runtime.
- Reuse merged Builder/Beacon API interfaces where applicable; keep the selection-event decision in SPEC-01 separate from payload-attributes work.

### Readiness and sync behavior

- Retain source-BN startup diagnostics and gate new runtime bid work on current source readiness, non-optimistic inputs, and the configured payload source's availability. The BN still validates submissions independently.
...
- Return a precise syncing or unavailable outcome when new bid work cannot proceed. Keep observation and already-committed reveal obligations separate from the decision to start another bid.
...
The active #9970 store provides explicit-key lookup and slot pruning. Consumers must handle missing material and verify the selected bid against retained material; the broader capacity, copying, first-write, and structured-error guarantees proposed in #63 are not accepted store behavior. Never rebuild a different payload after selection. In-memory retention is accepted for the first loop; durable recovery and multi-instance transfer remain separate work.
...
| Source-BN client and readiness | [#9781](https://github.com/ChainSafe/lodestar/pull/9781), [#9827](https://github.com/ChainSafe/lodestar/pull/9827), [#9868](https://github.com/ChainSafe/lodestar/pull/9868), BN sync helpers | The standard response path, inert wait/retry for unknown and pending keys, cancellation, identity checks, readiness diagnostics, abort-loop/logging fixes, transient identity handling, and tracker tests are merged | Keep historical thread reconciliation separate from TEST-01; runtime bid consumers must use current readiness and payload-source availability, without adding a BN preparation route |
...
| Direct-Engine preparation and payload retention | [#9958](https://github.com/ChainSafe/lodestar/pull/9958), [#9973](https://github.com/ChainSafe/lodestar/pull/9973), [#9970](https://github.com/ChainSafe/lodestar/pull/9970) | The source, orchestration, and store boundaries are under review; the BN's existing production path is reference code, not the Builder's payload owner | Wire authoritative BN inputs into the Builder-owned build/store lifecycle; qualify cancellation, retention, and Engine ownership before claiming a working runtime |
...
head_v2                     → EMPTY/FULL head outcome
```
...
| [`nflaig/builder`](https://github.com/ChainSafe/lodestar/tree/nflaig/builder) - complete Builder development branch | Draft proof of concept at `99fd8fa9ad` | Implements enriched `block` fields and a single compatibility fetch as end-to-end evidence. API-02 remains the stronger bounded complete-block fallback |
...
| [#9931](https://github.com/ChainSafe/lodestar/pull/9931) / [#9932](https://github.com/ChainSafe/lodestar/pull/9932) | #9931 ready; #9932 merged 9 September | API-02 review is independent of the direct-Engine stack; TEST-01 is complete |
...
| [#9970](https://github.com/ChainSafe/lodestar/pull/9970) / [contribution #9](https://github.com/markolazic01/lodestar/pull/9) | #9970 merged, including test-only #9 | Keep #9970's initial simple store. #9 adds agreed pruning/lookup tests; broader capacity, copying and first-write behavior remains unaccepted |
| [#9974](https://github.com/ChainSafe/lodestar/pull/9974) / [contribution #10](https://github.com/markolazic01/lodestar/pull/10) / [#9975](https://github.com/ChainSafe/lodestar/pull/9975) / [#9976](https://github.com/ChainSafe/lodestar/pull/9976) | Policy ready with #10 merged into its branch; broader validation open in LOD-64; ledger ready; preferences ready with event and lifecycle wiring | Pure bid foundations. They narrow BID-CORE-01 but do not complete a working bid loop |
...
| [#9761](https://github.com/ChainSafe/lodestar/pull/9761) | Merged 10 September | Gloas compliance coverage; not proof of a complete Builder lifecycle |
...
| [#5594](https://github.com/ethereum/consensus-specs/pull/5594) / [Lodestar #9972](https://github.com/ChainSafe/lodestar/pull/9972) | Merged September 3 | Reject a bid whose block hash equals its parent hash. Reuse the specification vectors and Lodestar implementation |
