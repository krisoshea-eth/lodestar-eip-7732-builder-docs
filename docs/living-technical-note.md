...
| Builder implementation | Initial [`@lodestar/builder`](https://www.npmjs.com/package/@lodestar/builder) package merged in [#9758](https://github.com/ChainSafe/lodestar/pull/9758); [#9781](https://github.com/ChainSafe/lodestar/pull/9781) is open, review-required, and not draft at `ce6e28d47d` with 28 commits and four unresolved threads; `SIGN-01`, `CLI-01`, and `API-01` are Done by project decision, `REVIEW-01` and `TEST-01` are In Progress, and `MET-01` is Todo |
...
- [Lodestar #9781](https://github.com/ChainSafe/lodestar/pull/9781) is open, review-required, and not draft at head `ce6e28d47d7d1ac81b64be8e2709118e73d6593c`, with 28 commits and four unresolved review threads on August 10. It adds Builder identity/status tracking, BN and EL readiness polling, `--executionFeeRecipient`, request timeout wiring, and identity/tracker tests. The reviewed later-Builder direction is to keep an unknown configured key inert and retry with cancellation until a later deposit or activation can be discovered, while treating a returned non-active lifecycle status as a distinct operator-visible result. Remaining review work covers that implementation, standard API-client response handling with bounded useful errors, and the current assumption that Gloas-capable clients expose the health endpoint.
...
- [#9781](https://github.com/ChainSafe/lodestar/pull/9781), open, review-required, and not draft at `ce6e28d47d`: Gate-A Builder identity, readiness, configuration, lifecycle implementation, and tests;
...
| Source-BN client and readiness | [#9781](https://github.com/ChainSafe/lodestar/pull/9781), BN sync helpers | Open at `ce6e28d47d` with four unresolved threads. Identity/tracker tests cover the main returned-state and polling cases; unknown-key polling/later activation, cancellation, readiness recovery, and bounded error detail remain follow-up | Implement the reviewed inert wait/retry path for an unknown configured key, preserve returned non-active status separately, retain the BN-owned preparation guard, and finish the missing test matrix in TEST-01 |
...
| [#9781](https://github.com/ChainSafe/lodestar/pull/9781) — Builder identity/readiness/CLI | Open, review-required, not draft at `ce6e28d47d`; 28 commits and four unresolved threads | CLI-01/API-01 remain closed by project decision. REVIEW-01 owns the reviewed unknown-key lifecycle implementation and remaining review/merge work; TEST-01 and MET-01 retain their separated scopes |
