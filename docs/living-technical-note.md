...
The [11 September reconciliation](reviews/2026-09-11-reconciliation.md) supersedes moving statuses in the snapshots below. API-02, the ledger, preference subscription, Marko's store/policy and the envelope metrics have merged. #9979/#9980/#9982 no longer wait for the ledger to merge; their actual conflicts were corrected incrementally, but their draft presentation still includes historical parent files. #9958 remains open, with the fork-correlated request type retained after testing the suggested simplification. #10054 now changes only the existing simulation helper and passed a fresh full local Docker multifork run. That run does not prove the Gloas bid/selection/reveal lifecycle.
