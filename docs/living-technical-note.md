...
| Builder implementation | Initial [`@lodestar/builder`](https://www.npmjs.com/package/@lodestar/builder) package merged in [#9758](https://github.com/ChainSafe/lodestar/pull/9758); identity, readiness, fee-recipient wiring, and status tracking merged in [#9781](https://github.com/ChainSafe/lodestar/pull/9781) as `2a04194b900e`; API-02 integrated that baseline in fork PR [#48](https://github.com/krisoshea-eth/lodestar/pull/48). `REVIEW-01` remains In Progress only for post-merge thread-marker and follow-up reconciliation, `TEST-01` is In Progress, API-02 is In Review, and `MET-01` is Todo |
...
| Last reconciliation | August 14, 2026: Lodestar v1.46.0, merged Builder PR #9781 and its full review-thread inventory, open follow-ups #9819/#9827, integrated API-02 implementation and tests, implementation plan, Linear, and GitHub issue mirror |
| Next milestone | Open API-02 against `ChainSafe/lodestar:unstable`, finish `BASELINE-01` and `ENV-01`, reconcile #9781's review markers and #9827 in `REVIEW-01`, and continue the separated `TEST-01` work before activating the reviewed BN preparation/candidate design while `MET-01` proceeds separately |
...
These items were reconciled against the August 10 partial monitoring report, confirmed Lodestar-team guidance, and direct live primary-source checks through August 14. The partial run did not advance unavailable source cursors or establish runtime health. Re-check them after each baseline bump.
...
- `SIGN-01`, `CLI-01`, and `API-01` are complete in the project board. [Lodestar #9781](https://github.com/ChainSafe/lodestar/pull/9781) merged as `2a04194b900ef972c6f469d06017d5c972be5714` after Nico approved head `3fe4ef4980645144668f8f634884c72a69f83677` and required checks passed. `REVIEW-01` now keeps only the post-merge thread-marker and follow-up reconciliation visible; `TEST-01` and `MET-01` own the remaining cross-cutting test and metric work.
...
- Twelve #9781 review-thread markers remain unresolved in GitHub despite the merge: five are outdated, two record accepted or already-applied behavior, one points to open test-helper deduplication issue [#9819](https://github.com/ChainSafe/lodestar/issues/9819), and four logging or abort-loop nits are carried by open follow-up [#9827](https://github.com/ChainSafe/lodestar/pull/9827). They remain explicit `REVIEW-01` cleanup rather than being silently marked resolved. Broader readiness recovery, bounded diagnostic, genesis logging, and CLI coverage remain in `TEST-01`.
...
Important moving inputs on August 14:
...
- [#9827](https://github.com/ChainSafe/lodestar/pull/9827), open and ready for review: Builder abort-loop and logging follow-ups from the final #9781 review;
...
- **Source-BN readiness:** [#9781](https://github.com/ChainSafe/lodestar/pull/9781) is merged. Reconcile its twelve remaining review-thread markers, review open follow-up [#9827](https://github.com/ChainSafe/lodestar/pull/9827), retain [#9819](https://github.com/ChainSafe/lodestar/issues/9819) as separate test-helper cleanup, and close broader regressions through `TEST-01`.
...
Status reflects the August 14 reconciliation. The merged implementation plan remains the delivery source of truth; this map highlights moving code seams and upstream work.

| Area | File / PR | Current understanding | Builder follow-up |
|---|---|---|---|
| Builder package and CLI | [#9758](https://github.com/ChainSafe/lodestar/pull/9758), [#9766](https://github.com/ChainSafe/lodestar/pull/9766), [#9781](https://github.com/ChainSafe/lodestar/pull/9781) | Initial package, TypeScript follow-up, and #9781 identity/status tracking, readiness, fee-recipient, and timeout wiring are merged; #9827 carries final abort-loop and logging nits | Keep CLI-01/API-01 closed by project decision; reconcile the remaining review markers and #9827 in REVIEW-01 while TEST-01/MET-01 continue separately; keep generated CLI docs hidden until functional |
...
| Source-BN client and readiness | [#9781](https://github.com/ChainSafe/lodestar/pull/9781), [#9827](https://github.com/ChainSafe/lodestar/pull/9827), BN sync helpers | #9781 merged the standard response path, inert wait/retry for unknown and pending keys, cancellation, identity checks, readiness diagnostics, and tracker tests. #9827 is open for abort-loop and logging nits; broader readiness and CLI regressions remain follow-up | Reconcile the merged review markers and #9827 in REVIEW-01, retain the BN-owned preparation guard, and finish the missing test matrix in TEST-01 |
...
| `REVIEW-01` | In progress | #9781 merged with Nico approval and passing checks; reconcile twelve remaining GitHub thread markers, open follow-up #9827, and separate test-helper issue #9819 |
...
| `API-02` | In review | Fork draft #48 has integrated merged #9781; docs PRs #13 and #14 are merged; the upstream Lodestar PR and `ENV-01` real-BN evidence remain before closure |
...
| [#9781](https://github.com/ChainSafe/lodestar/pull/9781) - Builder identity/readiness/CLI | Merged as `2a04194b900e` after Nico approval and passing checks; twelve review-thread markers remain in the UI | CLI-01/API-01 remain closed by project decision. REVIEW-01 owns explicit thread-marker reconciliation, #9827, and the separate #9819 follow-up; TEST-01 and MET-01 retain their separated scopes |
| [#9827](https://github.com/ChainSafe/lodestar/pull/9827) - Builder review follow-ups | Open, ready for review | Carries abort-loop and logging nits referenced from the final #9781 review; reconcile it through REVIEW-01 |
