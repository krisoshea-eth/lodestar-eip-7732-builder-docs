...
| Builder implementation | Initial [`@lodestar/builder`](https://www.npmjs.com/package/@lodestar/builder) package merged in [#9758](https://github.com/ChainSafe/lodestar/pull/9758); [#9781](https://github.com/ChainSafe/lodestar/pull/9781) is open, review-required, and not draft at `7386db0808` with 30 commits and four unresolved threads; `SIGN-01`, `CLI-01`, and `API-01` are Done by project decision, `REVIEW-01` and `TEST-01` are In Progress, and `MET-01` is Todo |
...
These items were reconciled against the August 10 partial monitoring report, confirmed Lodestar-team guidance, and direct live primary-source checks on August 10. The partial run did not advance unavailable source cursors or establish runtime health. Re-check them after each baseline bump.
...
- [Lodestar #9781](https://github.com/ChainSafe/lodestar/pull/9781) is open, review-required, and not draft at head `7386db08081ce635662d6898c925036cd2015b71`, with 30 commits and four unresolved review threads on August 10. It adds Builder identity/status tracking, BN and EL readiness polling, `--executionFeeRecipient`, request timeout wiring, and identity/tracker tests. Its two latest commits replace the manual Builder-response check with the API client's standard `.value()` path and fix the asynchronous API-error response mocks. The reviewed later-Builder direction is to keep an unknown configured key inert and retry with cancellation until a later deposit or activation can be discovered, while treating a returned non-active lifecycle status as a distinct operator-visible result. Remaining review work covers the later-Builder lifecycle, bounded useful error detail, and the current assumption that Gloas-capable clients expose the health endpoint.
...
Important moving inputs on August 10:

- [#9781](https://github.com/ChainSafe/lodestar/pull/9781), open, review-required, and not draft at `7386db0808`: Gate-A Builder identity, readiness, configuration, lifecycle implementation, and tests;
...
Status reflects the August 10 reconciliation. The merged implementation plan remains the delivery source of truth; this map highlights moving code seams and upstream work.

| Area | File / PR | Current understanding | Builder follow-up |
|---|---|---|---|
| Builder package and CLI | [#9758](https://github.com/ChainSafe/lodestar/pull/9758), [#9766](https://github.com/ChainSafe/lodestar/pull/9766), [#9781](https://github.com/ChainSafe/lodestar/pull/9781) | Initial package and TypeScript follow-up are merged; #9781 is open, review-required, and not draft at `7386db0808` with identity/status tracking, readiness, fee-recipient and timeout wiring | Keep CLI-01/API-01 closed by project decision; resolve #9781 feedback, lifecycle implementation, documentation, and merge in REVIEW-01 while TEST-01/MET-01 continue separately; keep generated CLI docs hidden until functional |
...
| Source-BN client and readiness | [#9781](https://github.com/ChainSafe/lodestar/pull/9781), BN sync helpers | Open at `7386db0808` with four unresolved threads. Standard `.value()` response handling and asynchronous error-response mocks now match the reviewed direction; identity/tracker tests cover the main returned-state and polling cases, while unknown-key polling/later activation, cancellation, readiness recovery, and bounded error detail remain follow-up | Implement the reviewed inert wait/retry path for an unknown configured key, preserve returned non-active status separately, retain the BN-owned preparation guard, and finish the missing test matrix in TEST-01 |
...
| [#9781](https://github.com/ChainSafe/lodestar/pull/9781) — Builder identity/readiness/CLI | Open, review-required, not draft at `7386db0808`; 30 commits and four unresolved threads. The standard `.value()` response path and async error mocks are now in the branch | CLI-01/API-01 remain closed by project decision. REVIEW-01 owns the reviewed unknown-key lifecycle implementation and remaining review/merge work; TEST-01 and MET-01 retain their separated scopes |
