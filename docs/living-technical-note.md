...
| Last reconciliation | September 2, 2026: all 42 changed files in `nflaig/builder` at `99fd8fa9ad`, Lodestar `unstable` at `d55b13cf5e`, current Builder PRs through #9986 plus fork drafts #77 and #80, directly relevant consensus/API/Buildoor changes, the BASELINE-01 matrix, and all 76 current Linear issues. All 76 Linear titles have GitHub issue mirrors; closed legacy `NICO-01` and unlabelled administrative `PRESENTATION-01` remain GitHub-only. Marko's attribution and active items and Kris's active implementation items have verified workflow fields; the four new payload-attributes and runtime children remain intentionally unassigned |
