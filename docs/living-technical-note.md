...
| Last reconciliation | September 2, 2026: all 42 changed files in `nflaig/builder` at `99fd8fa9ad`, Lodestar `unstable` at `7d85330f92`, current Builder PRs through #9982, directly relevant consensus/API/Buildoor changes, the BASELINE-01 matrix, and all 65 current Linear issues. GitHub issue bodies are mirrored; closed legacy `NICO-01` and unlabelled administrative `PRESENTATION-01` remain GitHub-only, and GitHub Project field verification requires refreshed `read:project` authorization |
