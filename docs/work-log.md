...
- Audited the runtime event boundary on Node 24.13.0. Lodestar uses the npm `eventsource` fallback; the Builder CLI currently pins one BN URL; shared-client multi-URL fallback would need explicit source affinity; and Lodestar SSE frames provide no `id` for `Last-Event-ID` replay. Routed connected/reconnecting shutdown evidence to `ENV-01`, bounded same-source reconciliation to `REL-01`, multi-BN provenance to LOD-37, and long-gap replay/reorg work to LOD-41.
