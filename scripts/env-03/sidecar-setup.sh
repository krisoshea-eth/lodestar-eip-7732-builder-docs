...
# run inside the lodstar repo, have `builder-dev.yaml` in there
...
# start devnet
kurtosis run --enclave builder-dev --args-file ./builder-dev.yaml github.com/ethpandaops/ethereum-package
...
if [ ! -d "./test-builder" ]; then
  ...
  # move into the dir
  mkdir -p ./test-builder
  cp "$(find ./builder-keys/keys -name voting-keystore.json | head -n1)" ./test-builder/keystore.json
  cp "$(find ./builder-keys/secrets -type f | head -n1)" ./test-builder/password.txt
fi

# download network config
kurtosis files download builder-dev el_cl_genesis_data ./netcfg

echo
echo "devnet up. run the sidecar in another terminal:"
echo
echo 'LODESTAR_PRESET=minimal ./lodestar builder \'
echo '  --keystore ./test-builder/keystore.json \'
echo '  --keystorePassword ./test-builder/password.txt \'
...
echo '  --paramsFile ./netcfg/config.yaml'
echo
